...
/*--------------------------------------------------*/
/*!
  * @fn     static int32_t check_param_sampling_period(adc_sampling_period_t param, uint32_t sclk)
  * @brief  Check the Sampling Period's parameter and  SCLK period.
  * @param[in] param :Sampling Period's parameter, SCLK period
  ...
  */
/*--------------------------------------------------*/
static int32_t check_param_sampling_period(adc_sampling_period_t param,  uint32_t sclk)
{
    int32_t result = ADC_PARAM_NG;

    switch (param) 
    {
    case ADC_SAMPLING_PERIOD_1:
		if((sclk & ADC_MOD1_AVDD5_3V_MASK) && (sclk <= ADC_MOD1_AVDD5_3V_SCLK_38)){
				result = ADC_PARAM_NG;
		}else if(sclk <= ADC_MOD1_AVDD5_5V_SCLK_14){
				result = ADC_PARAM_NG;
		}else{
			result = ADC_PARAM_OK;
		}
    ...
    case ADC_SAMPLING_PERIOD_2:
		if((sclk & ADC_MOD1_AVDD5_3V_MASK) && (sclk <= ADC_MOD1_AVDD5_3V_SCLK_28)){
				result = ADC_PARAM_NG;
		}else{
				result = ADC_PARAM_OK;
		}
	...
	case ADC_SAMPLING_PERIOD_3:
		if((sclk & ADC_MOD1_AVDD5_3V_MASK) && (sclk <= ADC_MOD1_AVDD5_3V_SCLK_18)){
				result = ADC_PARAM_NG;
		}else{
				result = ADC_PARAM_OK;
		}
    ...
    case ADC_SAMPLING_PERIOD_4:
		if((sclk & ADC_MOD1_AVDD5_3V_MASK) && (sclk <= ADC_MOD1_AVDD5_3V_SCLK_14)){
				result = ADC_PARAM_NG;
		}else{
				result = ADC_PARAM_OK;
		}
		break;
    case ADC_SAMPLING_PERIOD_16:
	case ADC_SAMPLING_PERIOD_64:
	case ADC_SAMPLING_PERIOD_128:
        result = ADC_PARAM_OK;
        break;
    default:
        /* no process */
...
TXZ_Result adc_init(adc_t *p_obj)
{
    ...
    if (((void*)(p_obj) == (void*)0) ||
        ...
        (check_param_sampling_period(p_obj->init.clk.exaz, p_obj->init.mod1) == ADC_PARAM_NG) ||
        (check_param_prescaler_output(p_obj->init.clk.vadcld) == ADC_PARAM_NG))
    {
        result = TXZ_ERROR;
    }
    else
    {
		...
		while(time < ADC_MOD0_DACON_WAIT_TIME)
		{
			time++;
		}
        /*--- ADxCLK  ---*/
        p_obj->p_instance->CLK  = ((uint32_t)p_obj->init.clk.exaz | (uint32_t)p_obj->init.clk.vadcld);
        /*--- ADxMOD1 ---*/
...
TXZ_Result adc_deinit(adc_t *p_obj)
{
    ...
    else
    {
        ...
        /*--- ADxMOD1 ---*/
        p_obj->p_instance->MOD1  = ADC_MOD1_AVDD5_5V_SCLK_16;
        /*--- ADxMOD2 ---*/
...
TXZ_Result adc_channel_setting(adc_t *p_obj, uint32_t ch, adc_channel_setting_t *p_setting)
{
    ...
    if (((void*)(p_obj) == (void*)0) ||
        ...
        (ch >= ADC_NUM_MAX) ||
        (check_param_interrupt((adc_int_t)p_setting->interrupt) == ADC_PARAM_NG) ||
        (check_param_type((adc_conversion_t)p_setting->type) == ADC_PARAM_NG) ||
        (check_param_ain((adc_ain_range_t)p_setting->ain, ADC_AIN_RANGE_MIN, ADC_AIN_RANGE_MAX) == ADC_PARAM_NG))
    {
        result = TXZ_ERROR;
    }
    else
...
TXZ_Result adc_cmp_init(adc_t *p_obj, adc_cmpx_t *p_cmpx_t)
{
    ...
    else
    {
        ...
        if (p_cmpx_t->CMPEN == ADCMP0EN_DISABLE)
            p_obj->p_instance->CMPEN &= ~ADCMP0EN_ENABLE;
        if (p_cmpx_t->CMPEN == ADCMP1EN_DISABLE)
            p_obj->p_instance->CMPEN &= ~ADCMP1EN_ENABLE;
        /*------------------------------*/
        /*  Register Setting            */
        /*------------------------------*/
...
/*!
  ...
  * @note   When the 1ms interrupt execute interruption, every time is counted.
            ( The 1ms interrupt is using T32A timer. )
            The count value is being used for a judgment of timeout.
  * @attention This function is not available in interrupt.
  * @attention This function is not available for Continuation Conversion and Universal Trigger.
  */
/*--------------------------------------------------*/
